...
public class OKMCTSBasedPlayer extends SPPlayer { // simplified and ported from
    ...
    OKStateFeaturesLR1 features = new OKStateFeaturesLR1(); // Features for heuristic evaluation
    ...
    public OKMCTSBasedPlayer() {
        super("OKMCTSBasedPlayer");
    }
        ...
        for (int iter = 0; iter < numIterations; iter++) { // MCTS loop
            ...
            while (!state.isGameOver()) {
                ...
                if (nextNode.isChanceNode()) {
                    ...
                    int actionIndex = nextNode.action;
                    ArrayList<SPAction> actions = state.getLegalActions();

                    // if(actionIndex >= actions.size()){
                    //     System.out.println("ActionIndex: " + actionIndex + " Size: " + actions.size());
                    //     System.out.println(node.childrenStr(node.exploreCount, uctC));
